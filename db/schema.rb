...
ActiveRecord::Schema[8.1].define(version: 2026_07_25_173300) do
  ...
  create_table "profiles", force: :cascade do |t|
    ...
    t.bigint "user_id", null: false
    t.string "uuid", null: false
    t.index "user_id, lower((COALESCE(chat_bracket_before, ''::character varying))::text), lower((COALESCE(chat_bracket_after, ''::character varying))::text)", name: "index_profiles_on_user_id_and_chat_bracket_pair", unique: true, where: "((chat_bracket_before IS NOT NULL) OR (chat_bracket_after IS NOT NULL))"
