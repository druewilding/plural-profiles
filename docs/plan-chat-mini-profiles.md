...
- **Built**: `test/controllers/chat/mini_profiles_controller_test.rb` (9 tests) — never an "Edit profile"/"Edit group" link, for anyone including the owner; `View full profile page`/`View full group page` for *anyone* (owner included) only when `mini_profile_link_enabled?`, nothing at all for anyone otherwise; an overridden field (`mini_profile_pronouns_inherited: false`) shows the resolved chat value rather than the real one; unknown uuid / invalid `postable_type` 404; unauthenticated redirects to sign-in. Not yet covered here (left for a future pass, not blocking): avatar-shape-differs-from-fallback and blank-section-omitted cases — both already covered at the helper/partial level by `test/helpers/application_helper_test.rb` and the settings-page preview tests, just not re-asserted through this specific controller.
