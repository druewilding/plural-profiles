...
- **Chat popover controller/route — built** (step 7). `Chat::MiniProfilesController#show` at the existing `constraints subdomain: "chat"` route, `@postable`/`@own` resolution as originally planned.
...
- **New Stimulus controller** `app/javascript/controllers/mini_profile_popover_controller.js` — **built** (step 9), one small adaptation from the original spec below:
  ...
  - On trigger click: if the frame's `src` isn't set yet, set it to the `url` value (fires the Turbo fetch exactly once per message); `showPopover()`; then position the panel from the clicked trigger's `getBoundingClientRect()`, clamped to stay inside the viewport.
...
- **Built**: `test/controllers/chat/mini_profiles_controller_test.rb` (7 tests) — owner-vs-viewer link behavior (`Edit profile`/`Edit group` for the owner regardless of the boolean, `View full profile page` for a non-owner only when `mini_profile_link_enabled?`, nothing at all otherwise), an overridden field (`mini_profile_pronouns_inherited: false`) shows the resolved chat value rather than the real one, unknown uuid / invalid `postable_type` 404, unauthenticated redirects to sign-in. Not yet covered here (left for a future pass, not blocking): avatar-shape-differs-from-fallback and blank-section-omitted cases — both already covered at the helper/partial level by `test/helpers/application_helper_test.rb` and the settings-page preview tests, just not re-asserted through this specific controller.
- Also updated for the new click-to-popover behavior rather than link-based navigation: `test/models/chat/message_test.rb`'s broadcast-partial tests (now assert the `chat_mini_profile_path` trigger URL appears, not `profile_path`/`group_path`) and `test/controllers/chat/channels_controller_test.rb`'s message-author test (now asserts each message's trigger points at its own postable, not that the owner/non-owner see different link targets — that distinction lives inside the popover's fetched content now, not the message row).
