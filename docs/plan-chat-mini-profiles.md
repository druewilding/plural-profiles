...
- **New "Edit chat settings" page. Built** (`app/views/our/chat_identities/edit.html.haml`, step 5): one `form_with model: @postable, scope: :chat_identity, url: our_chat_identity_path(...)` (see Backend for why `scope:` is needed alongside `model:`), containing:
  - Chat proxy brackets (moved here unchanged from `_form.html.haml`).
  - The mini-profile avatar editor (parameterized `shared/avatar_editor_dialog` instance, with the `fallback_*` locals above).
  - For each of name/subtitle/pronouns(Profile)/tagline/description: a "Follow my profile" / "Set independently for chat" radio pair bound to the field's `_inherited` column (factored into a new `_field_toggle.html.haml` partial — see Order of implementation), with the live full-profile value shown alongside so the owner can see what they're inheriting without leaving the page. **Not yet conditionally shown/hidden** — that's step 6; both the "currently on your profile" and override blocks render unconditionally for now, so the page stays fully usable (if a bit busier-looking) before the JS lands.
  - Hearts picker for `mini_profile_heart_emojis` (Profile only) — same `heart-picker` Stimulus component as the full-profile picker, parameterized to `chat_identity[mini_profile_heart_emojis][]`; hand-written rather than routed through `_field_toggle` since it's a checkbox grid, not a single input.
  ...
  - A live preview panel — **currently a static server-rendered snapshot** (`render "chat/mini_profiles/mini_profile", postable: @postable, own: true`, correct as of page load) rather than a genuinely live one; it becomes reactive once step 6's Stimulus controller wires up the debounced `preview` fetch. The panel already carries `data: {"chat-identity-form-target": "preview"}` so step 6 has something to target without touching this markup again.
  - A one-line explanation at the top of the page: "This is what people see in chat when [name] posts — independent from the full profile page. Nothing here is filled in automatically except the name." (kept name-and-pronoun-neutral so the identical copy reads correctly for both a Profile and a Group.)
...
- **Main profile/group edit form. Built** (step 4a): `_form.html.haml`/`_form.html.haml` (groups) dropped the "Chat proxy brackets" `.form-group` (step 3) and gained a link next to the "Created" field: `link_to "Manage chat settings →", edit_our_chat_identity_path(profile.class.name, profile.uuid)`, only shown once the record is persisted (nested inside the existing `if profile.persisted?` block, alongside `created_at`).
...
4a. **Built** (once step 5's route existed to link to): the "Manage chat settings →" link in both `_form.html.haml` partials, next to the "Created" field, only shown once the record is persisted.
