...
- **Every field is independently overridable, including name — via an explicit per-field inherit/override choice, not just a blank/filled column.** Each chat-identity field (name, subtitle, pronouns, hearts, tagline, description) has its own "inherit from my main profile" vs. "set independently for chat" toggle. This is a deliberate two-mode design, not three: there's no separate "hidden" mode, because choosing "set independently" and simply leaving the override blank *is* hidden — one less concept for the owner to learn, and it composes with "omit the section if blank" (below) for free.
  - **Every field defaults to inherited (`true`), except `mini_profile_link_enabled` (`false`) and, per round 7 below, `mini_profile_description_inherited`** — per round 3. This replaces the round-1-derived "independent and blank unless opted in" default from earlier drafts of this plan. The reasoning: most people are fine with their chat identity matching their profile, and forcing everyone through an empty-by-default settings page before chat shows anything about them is more friction than the privacy problem warrants once the *real* mechanism for control — per-field override plus a live preview that shows exactly what's exposed — exists. Nothing is hidden from the owner's control; the settings page is where they go to see and adjust it, whenever they want, not only once at signup.
  - **Round 7 (reverses the original description-defaults-to-inherited call below): `mini_profile_description_inherited` now defaults to `false`, the one field-inherit flag that doesn't default to `true`.** Description is the field people are most likely to want to keep out of chat entirely, or write a shorter version of — of everything on a profile, it's the one most likely to be "an enormous sprawling mess of half-finished html" (see Background) not meant for casual chat viewing. Rather than accept that risk and rely purely on the scroll-area mitigation below, a fresh profile/group's chat description now starts genuinely blank until the owner deliberately writes one for chat, same as `mini_profile_link_enabled`. Implemented via the "rollback, edit the original migration in place, remigrate" approach (never a second corrective migration) — safe because this feature is still unshipped and branch-only, so no real data depends on the old default. The scroll-area treatment below is kept regardless, for whenever an owner does choose to inherit or write a long one.
  - *(Original round-3 reasoning, superseded by round 7 for this one field, kept for context):* description defaulted to inherited too, which meant it was possible to ship this feature and have the exact "sprawling mess" scenario from the top of this doc show up in chat by default, for anyone who hadn't visited the settings page. The mitigation wasn't meant to be a different default — it was a **UI treatment for long descriptions in the popover**: a `max-height` with `overflow-y: auto` (a scroll area, not hard truncation — nothing is cut off or silently dropped, it's just contained) on `.pop-description`/`.mini-profile-popover__description`. Chosen over truncate-with-"show more" because it needs no JS and never loses content.
  - **Also per round 7**: when a postable's resolved description is blank *and* `mini_profile_link_enabled?` is off, `.mini-profile__header` (which already bleeds edge-to-edge with the header background — see Frontend) is the only thing left in the popover, so its own trailing margin — normally there to separate it from the description/link below — is collapsed via a `:last-child` CSS rule instead of leaving dead empty space under the header. Since description now defaults to blank, this is the common case for a freshly-created profile/group, not an edge case.
...
- `mini_profile_description` (`text`, nullable) + `mini_profile_description_inherited` (`boolean`, `default: false, null: false` — the one field-inherit flag that defaults to `false`, per round 7 in Direction) — rendered with `formatted_description`; the popover contains it in a scrollable area rather than capping its length (see Direction).
