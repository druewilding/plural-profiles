...
- **`datetime_local_field` overrides now use the viewer's zone, not UTC** (updated after initial implementation) — the "Created at" override inputs in [profiles/_form.html.haml](app/views/our/profiles/_form.html.haml#L68-69) and [groups/_form.html.haml](app/views/our/groups/_form.html.haml#L38-39) dropped `.utc` and the "(UTC)" label. Since `created_at` is a time-zone-aware attribute, rendering with plain `.strftime` displays it in the request's `Time.zone` (resolved by the `around_action`), and assigning the submitted string back interprets it in that same zone (`Time.zone.parse` semantics), converting correctly to UTC on save. The "unchanged value" comparisons in `profiles_controller.rb`/`groups_controller.rb` (`profile_params`/`group_params`) were updated in lockstep to drop `.utc` too, so the "did the user actually touch this field" check still compares like-for-like.
