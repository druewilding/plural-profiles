...
- **Group nesting** — groups can contain other groups. Each parent→child link has a configurable inclusion mode; when the child has sub-groups of its own, the mode controls how much of its sub-tree appears in the parent:
  ...
  - **Selected** — only specific direct sub-groups of the child are included in the parent, along with the child's direct profiles; those sub-groups' own children are not automatically pulled in
...
- `selected` — only specific direct sub-groups of the child (stored as `included_subgroup_ids` on the join record) are included in the parent, along with the child's direct profiles. Those sub-groups' own children are not automatically pulled in, and sub-groups not in the list are excluded.
